...
class H2O_Pickle_Model():
    '''
    H2O_Pickleable_Model is a class that wraps the H2O model such that it can be serialized with Pickle.

    Example:
    To pickle a model, 
    >> model # Assume that this model is trained with H2O.
    >> pic_model = H2O_Pickleable_Model(model)
